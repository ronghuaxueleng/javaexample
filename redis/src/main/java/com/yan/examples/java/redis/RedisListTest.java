...
//		jedis.auth(DATASOURCE_PASS);
	...
	public void testLPushAndLPushX() {
		...
		/**
		 * 多个值lpushx会报错 redis.clients.jedis.exceptions.JedisDataException: ERR wrong
		 * number of arguments for 'lpushx' command
	...
	public void testLInsert() {
		...
		// Assert.assertTrue(jedis.linsert("list",
		// BinaryClient.LIST_POSITION.AFTER,"d","insert")==5);
		Assert.assertTrue(jedis.del("list") == 1);
	}
	...
	public void testLrem() {
		 ...
		 * count 的值可以是以下几种：
		 * 
		 * count > 0 : 从表头开始向表尾搜索，移除与 value 相等的元素，数量为 count 。 count < 0 : 从表尾开始向表头搜索，移除与
		 * value 相等的元素，数量为 count 的绝对值。 count = 0 : 移除表中所有与 value 相等的值。
		 */
	...
	public void testRPopLPush() {
		/**
		 * 原子操作 将列表 source 中的最后一个元素(尾元素)弹出，并返回给客户端。 将 source 弹出的元素插入到列表 destination ，作为
		 * destination 列表的的头元素。
		 */
